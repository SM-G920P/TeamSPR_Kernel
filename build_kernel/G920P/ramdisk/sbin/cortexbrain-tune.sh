...
cortexbrain_background_process=$(cat /res/synapse/TeamSPR/cortexbrain_background_process);
cortexbrain_kernel=$(cat /res/synapse/TeamSPR/cortexbrain_kernel);
...
cortexbrain_nmi_auto=$(cat /res/synapse/TeamSPR/cortexbrain_nmi_auto);
...
if [ "$cortexbrain_background_process" == 1 ]; then
	(while [ 1 ]; do
sleep 5;
	SCREEN_OFF=$(cat /sys/class/backlight/panel/brightness); ## Adjusted for TeamSPR Galaxy S6 Exynos7420 kernel
